...
class Adaptor(SISPComponentBase):
    ...
    def iono_pi_max_read (self, fpath):
    ...
    def iono_pi_max_write (self, fpath, value):
        if os.path.isfile(fpath):
            f = open(fpath, 'w')
            f.write(str(value))
            f.close()
...
def add_common_adaptor_args(parser, **kw):
    ...
    add_arg(g, "--location", h="location of the adaptor  {D}", d=kw.get('location', 'BocSpace'), m='LOC')
    add_arg(g, "--site", h="site where the adaptor is deployed  {D}", d=kw.get('site', 'EWAIC-BocSpace'), m='SITE')
